...
int		check_buf(char *buf, const int fd)
{
    size_t i;
    int lineRead;
    char *before_n = (char *)malloc(sizeof(char) * ft_strlen(buf) + 1);

    before_n = ft_strcpy(before_n, buf);
    lineRead = 0;
    i = 0;
    while (*buf != '\n' && *buf != '\0' && *buf != EOF && i < BUFF_SIZE)
    {
...
int			fromReadToList(const int fd)
{
    ...
    //нужна проверка считана ли линия или закончено считывание
    return (1);
}
